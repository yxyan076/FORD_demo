...
from prompt import ROUND0_SYSTEM, ROUNDTABLE_DEBATE_SYSTEM, DEFAULT_QUESTION
from model import Openai_ChatModel, Openai_CompletionModel, Baichuan_Model, Qwen_Model


def display_speech(role, answer, argument, round):
    st.markdown(f"""
                <div style="display: flex; align-items: flex-start; justify-content: flex-start; margin-bottom: 20px;">
                    <span style="font-size: 1.5em; margin-right: 10px;">Round{round}<br>{role}</span>
...
with st.sidebar:
    ...
    if st.button('Add Baichuan Model➕'):
...
st.title("🏛️ LLM Roundtable Debate")
round0_prompt = st.text_area("Round0 Prompt", height=120, value=ROUND0_SYSTEM)
system_prompt = st.text_area("Debate Prompt", height=120, value=ROUNDTABLE_DEBATE_SYSTEM)
    ...
    for i in range(max_debate_rounds + 1):
        if i == 0:
            for j in range(st.session_state["debaters_count"]):
                ...
                display_speech(debaters[j].model, answer, argument, 0)
            ...
            viewpoints.append(answer)
            display_speech(debaters[debater_index].model, answer, argument, i)
            if len(set(viewpoints[-st.session_state["debaters_count"]:])) == 1:
                break
